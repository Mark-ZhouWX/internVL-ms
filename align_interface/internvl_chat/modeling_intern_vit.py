...
class InternVisionEmbeddings(nn.Cell):
    def __init__(self, config: InternVisionConfig):
        ...
        self.class_embedding = ms.Parameter(
            ops.randn((1, 1, self.embed_dim), dtype=self.dtype),
        )

        self.patch_embedding = nn.Conv2d(
            in_channels=3, out_channels=self.embed_dim, kernel_size=self.patch_size, stride=self.patch_size,
            has_bias=True, dtype=self.dtype
        )

        self.num_patches = (self.image_size // self.patch_size) ** 2
        self.num_positions = self.num_patches + 1

        self.position_embedding = ms.Parameter(ops.randn((1, self.num_positions, self.embed_dim), dtype=self.dtype))
...
class InternAttention(nn.Cell):
    ...
    def __init__(self, config: InternVisionConfig):
        ...
        self.qkv = nn.Dense(self.embed_dim, 3 * self.embed_dim, has_bias=config.qkv_bias, dtype=config.compute_dtype)
        self.attn_drop = nn.Dropout(p=config.attention_dropout)
        self.proj_drop = nn.Dropout(p=config.dropout)

        self.qk_normalization = config.qk_normalization

        self.proj = nn.Dense(self.embed_dim, self.embed_dim, dtype=config.compute_dtype)
    ...
    def _naive_attn(self, x):
        ...
        attn = ops.softmax(attn, axis=-1, dtype=self.config.compute_dtype)
...
class InternMLP(nn.Cell):
    def __init__(self, config: InternVisionConfig):
        super().__init__()
        self.config = config
        self.fc1 = nn.Dense(config.hidden_size, config.intermediate_size, dtype=config.compute_dtype)
        self.fc2 = nn.Dense(config.intermediate_size, config.hidden_size, dtype=config.compute_dtype)
...
class InternVisionEncoderLayer(nn.Cell):
    def __init__(self, config: InternVisionConfig, drop_path_rate: float):
        ...
        self.norm1 = NORM2FN[self.norm_type]([self.embed_dim], epsilon=config.layer_norm_eps, dtype=config.compute_dtype)
        self.norm2 = NORM2FN[self.norm_type]([self.embed_dim], epsilon=config.layer_norm_eps, dtype=config.compute_dtype)

        self.ls1 = ms.Parameter(config.initializer_factor * ops.ones(self.embed_dim, dtype=config.compute_dtype))
        self.ls2 = ms.Parameter(config.initializer_factor * ops.ones(self.embed_dim, dtype=config.compute_dtype))

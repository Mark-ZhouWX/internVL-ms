...
from mindspore.dataset import ConcatDataset, WeightedRandomSampler
...
def build_transform(is_train, input_size, pad2square=False, normalize_type='imagenet'):
    ...
    if is_train:  # use data augumentation
        transform = T.Compose([
            ...
            T.RandomChoice([jpeg_degrade_functions[quality] for quality in qualities]),
            vision.Resize((input_size, input_size), interpolation=Inter.BICUBIC),
            vision.ToTensor(),
        ...
        if pad2square is False:  # now we use this transform function by default
            ...
            transform = T.Compose([
                (lambda img: img.convert('RGB') if img.mode != 'RGB' else img),
                (lambda img: expand2square(img, tuple(int(x * 255) for x in MEAN))),
                vision.Resize((input_size, input_size), interpolation=Inter.BICUBIC),
                vision.ToTensor(),
                vision.Normalize(mean=MEAN, std=STD, is_hwc=False)
            ])
...
def preprocess(
        template_name,
        sources,
        tokenizer: mindnlp.transformers.PreTrainedTokenizer,

...
class LazySupervisedDataset:
                ...
                if 'length' in data_item:
                    ...
                    if str_length not in self.conv2length:
                        token_length = tokenizer(
                            conversations, return_tensors='ms', padding=False, truncation=False,
                        ).input_ids.shape[1]
    ...
    def multi_modal_get_item(self, data_item):
        ...
        image_token_total_num = self.num_image_token * num_patches
        img_context_token_id = self.tokenizer.convert_tokens_to_ids(IMG_CONTEXT_TOKEN)
        img_context_token_start = np.equal(ret['input_ids'][0], img_context_token_id).argmax(axis=0)  # (bs,)
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        pixel_values = ms.ops.stack(pixel_values)
    ...
    def pure_text_get_item(self, data_item):
        ...
        ret = dict(
            ...
            attention_mask=ret['attention_mask'][0],
            pixel_values=pixel_values,
            image_flags=ms.tensor([0] * num_patches, dtype=ms.int64)
        )
        return ret
...
def main():
    ...
    # logger.warning(
    #     f'Process rank: {training_args.local_rank}, device: {training_args.device}, n_gpu: {training_args.n_npu}'
    ...
    if model_args.model_name_or_path is not None:
        ...
        vision_model = InternVisionModel.from_pretrained(
            model_args.vision_path, torch_dtype=ms.bfloat16, config=vision_config)
